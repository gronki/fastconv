program fastconv_test
   ...
   integer :: i
   integer(int64), parameter :: array_sizes(*) = [(nint(1024 * 10**(i * 0.5), kind=int64), i = 0, 8)]
   ...
   subroutine run_test_1(test_title, conv)
      ...
      iter_array_sizes: do i = 1, size(array_sizes)
         ...
         iter_kernel_sizes: do j = 1, size(kernel_sizes)
            ...
            reps = max(1, nint(&
               3 * (real(maxval(array_sizes))**0.8 * real(maxval(kernel_sizes))**0.5) &
               / (real(array_sizes(i))**0.8 * real(kernel_sizes(j))**0.5)&
               ))

            ! print '(I0," ",I0," ",I0)', array_sizes(i), kernel_sizes(j), reps
